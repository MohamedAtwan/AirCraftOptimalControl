...
class Dynamics:
	...
	def dragForce(self, xx):
		...
		dD_x[6,0] = rho*V*dV_x[6,0]*S*(Cd0 + Cda * alpha**2) + rho*(V**2)*S*Cda*alpha
	...
	def acc_vector(self,xx,uu):
		...
		Vd = (1/m) * (-D-m*g*np.sin(xx[6,0]) + uu[0,0]*np.cos(alpha))
	...
	def step(self,xx,uu):
		...
		xxp[3] = xx[3,0] + dt * (-1*(Vd/m)*np.sin(xx[6,0])-V*xx[7,0]*np.cos(xx[6,0]))
		...
		for i in range(0,8):
			fx[i,3] = dt*(-1*(dVd_x[i,0]/m)*np.sin(xx[6,0])-dV_x[i,0]*xx[7,0]*np.cos(xx[6,0]))
		fx[3,3] +=1
		...
		for i in range(0,8):
			fx[i,7] = dt * ((1/m) * (rho*dVd_x[i,0]*S*Cla*alpha+rho*Vd*S*Cla*grad_alpha[i,0] +
					  ...
					  ((uu[0,0]*dV_x[i,0])/V**2)*xx[7,0]*np.sin(xx[6,0]) - ((dVd_x[i,0]*V-dV_x[i,0]*Vd)/V**2)*xx[7,0])
		fx[7,7] += 1
		...
		fu[0,3] = dt * (-1*(dVd_u[0,0]/m)*np.sin(xx[6,0]))
		fu[1,3] = dt * (-1*(dVd_u[1,0]/m)*np.sin(xx[6,0]))
